...
Note that at the end of each state's task list is a final task, e.g., `states/log/WI.ind`, that we didn't directly specify in the task_plan. These recipes for the `complete` step were generated in `create_task_plan` as described above. These .ind files representing the completeness of each task are also used as the dependencies of the final target, `task_plan_1.ind_promise`, which describes all targets that must be complete to consider the whole task batch complete. In this case, we're using the default `scipiper` combiner `combine_to_ind` to generate the final target, which simply adds the final step from each task (in this case, the `states/log/[statename].ind` files) and hash to `states/log/task_plan_1.ind`: 

![](default_combiner.PNG)

See the section on combiners for additional flexibility in ways to combine tasks into a final target.
...
loop_tasks(
  task_plan=task_plan_1,
  task_makefile='task_plan_1.yml', force = TRUE)
```
...
non_group_targets <- tail(list_all_targets('task_plan_1.yml'), -1L)
scmake(non_group_targets, 'task_plan_1.yml', force = TRUE)
```
...
```{r delete_all2}
non_group_targets <- tail(list_all_targets('task_plan_1.yml'), -1L)
scdel(non_group_targets, remake_file='task_plan_1.yml')
```
...
### Using task combiners

Occassionally at the end of a task plan, you'll want to combine all individual tasks into a final product. An example might be that you want to combine several NWIS data pulls into a single file, and upload it to a shared cache. Or, you want to take multiple `png` files and turn them into a GIF or report. Arguments withing `create_task_makefile` allow you to write combiner functions that have access to the final step in each task and the final target name. In this example, we want to create a GIF out of several state-specific png files. We'll write our own combiner called `combine_to_gif` which will create the final target `figures/all_states.gif` and the corresponding indicator file.

![](custom_combiners.PNG)

Note that for the combiner to receive the correct step name (in this case, the `png` files), we need to specify the proper final step and ensure each task does not have a indicator file. In this case, we've set `final_steps = visualize` and `add_complete = FALSE` in our `create_task_plan` function.

Now, we have a GIF that combines all of our state plots. 
